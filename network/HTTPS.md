...
인터넷이 발달하면서 많은 데이터들이 클라이언트와 서버 사이에 오가게 되었고, 이에 데이터 보안도 같이 발전하게 되었다. 초기에는 비공개키 암호화 방식을 쓰다가, 그 불편함과 문제점이 발견되면서 SSL에서 사용하는 것과 같은 공개키 암호화 방식을 사용하게 되었다.

### 비공개 키 암호화(= 대칭 키)

대칭키를 사용한다는 것은 암호화/복호화(암호화 된 것을 해독) 할 때 사용하는 키가 같다는 것이다. 즉, 송신자와 수신자가 동일한 키를 가지고 있어야 한다는 의미이다.
...
비공개 키 암호화의 문제점으로 인해 나오게 된 것이, 비대칭 키를 이용한 공개키 암호화이다. 암호화하는 키와 복호화 하는 키를 다르게 두어(비대칭적), 하나의 키가 공개되어도 보안이 깨지지 않는 방법을 고안한 것이다.
비공개키 암호화 방법을 쓰면, 사전에 키를 수신자에게 전달했어야 했다.
하지만, 이 공개 키 암호화를 쓰면 공개 채널 상에서 통신할 때 전달하고자 하는 데이터와 함께 키를 전달할 수 있는 것이다.
어떻게 이런 방법이 가능하게 된 것일까?
공개 키 암호화에는 **public key**와 **private key**가 존재한다.
...
우리는 HTTPS가 HTTP의 보안이 강화된 버전이고, 이때 SSL/TLS는 공개키 암호화 방식을 사용한다는 것을 이해했다.
...
앞의 공개 키 암호화에서 다룬 두번째 **데이터 발행자 증명 및 위조 방지** 사용분야를 생각해보자.
...
마지막으로 가장 많이 사용되는 공개키 암호화 알고리즘 중 **RSA 암호화**에 대해서 간단하게 설명하고자 한다.
