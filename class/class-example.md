...
그림판에 그림을 그리듯이 그래픽을 렌더링하는 것이 특징입니다.
...
도형을 이동시키려면 스톱모션 처럼 좌표가 이동될 때 마다 전체 그림을 다시 렌더링 해줘야합니다.
...
우리는 상태를 정의했으므로 그 상태가 렌더링할 요소들을 정의해줘야한다.
...
표시할 객체를 만들었으니 이를 렌더링해줄 상태를 정의해야합니다.
...
Canvas 를 활용해서 사각형을 움직이는 로직을 만들고 `class` 문법을 활용하여 이를 구조화 하였습니다.
